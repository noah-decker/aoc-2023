with open('inputs\day4_input.txt') as f:
    text = f.read()
atext = '''Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
...
for key,value in adict.items():
    ...
    w_num = nums[0]
    m_num = nums[1]
    count = score_game(w_num,m_num)
    for i in range(key+1,key+count+1):
        if i in adict:
            adict[i][1] = adict[i][1]+1*(adict[key][1])
print(adict)
for key in adict:
    score = score + adict[key][1]
print(score)
